                ...
                <View style={styles.monthContainer}>
                    {props.month_data.map((item, index) =>
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                setMonth(item);
                                props.onChange({item,"year":year})
                                props.onChangeMonth(month);
                                props.onChangeYear(year);
                                props.onChangeDate(item.name + '/' + year);
                                props.close()
                            }}
                            style={[styles.month, { width: (width / 3), backgroundColor: item.key==props.defaultIndex||item.key == month.key&& item.key==props.defaultIndex?colors.primary:'white' }]}>
                            <Text style={{ fontSize: fontScale(15),color:item.key==props.defaultIndex||item.key == month.key&& item.key==props.defaultIndex? 'white' : 'black' }}>{item.name}</Text>
                        </TouchableOpacity>
                    )}
